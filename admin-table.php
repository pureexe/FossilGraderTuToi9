...
function delete(){
	global $database;
	$user = $_POST['user'];
	if((mysql_query("DELETE FROM $database WHERE user_id=\"$user\"") or die(mysql_error())))
		echo "finish";
}
...
function update( $data ){
	...
	if( count($data)!=count($keys) || $user=='') return;
	
	$res = mysql_query("SELECT * FROM $database WHERE user_id=\"$user\"") or die(mysql_error());
  
	if( mysql_num_rows($res)==0 ){
		if(mysql_query("INSERT INTO $database (".implode(',',$keys).") VALUES (\"".implode('","',$data)."\")") or die(mysql_error()))	
			$insert_list[] = "<br/>\"+".json_encode(implode(", ", $data))."+\"";	
		else
			$insert_list[] = "<br/>[Error!]\"+".json_encode(implode(", ", $data))."+\"";
	}
	else{
		if(mysql_query("UPDATE $database SET ".implode(",",array_map("format", $keys, $data))." WHERE user_id=\"$user\"") or die(mysql_error()))
			...
			if( $download==false ){
				switch( $key ){
					...
					case 'type'   : echo"<select onkeypress='update_user(event);' style='padding:0; margin:0; border:2px; height:18; width:100%;'>";
											$typelist = array(USERTYPE_ADMIN,USERTYPE_SUPERADMIN);
...
function upload(){
	...
	if( $user!=''&&$name!=''&&$pass!=''&&$team!=''&&$type!='' ) update( array($user, $name, $pass, $team, $mail, $type) );
...
?>
<?php
$field = array("user_id"=>"username","name"=>"name","passwd"=>"password","grp"=>"group","email"=>"e-mail","type"=>"type");
$database = "user_info";
